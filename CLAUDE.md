...
- **CS-07**: **NEVER** use emoji, or Unicode that emulates emoji (e.g. check marks, X marks). Exceptions: (1) tests verifying multibyte character handling; (2) interactive TUI display elements (icons, spinners) in string literals passed directly to terminal-rendering functions within TUI modules, marked with `//codecheck:allow-emojis`.
